 ...
 *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px/1.45 -apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif}
 ...
 .ms-btn{background:var(--bg);color:var(--text);border:1px solid var(--line);border-radius:7px;padding:6px 26px 6px 8px;font-size:13px;font-weight:400;cursor:pointer;text-align:left;min-width:140px;position:relative;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:200px}
 .ms-btn::after{content:'▾';position:absolute;right:8px;top:50%;transform:translateY(-50%);color:var(--muted);font-size:10px;pointer-events:none}
 .ms-btn.active{border-color:var(--accent);color:var(--accent)}
 .ms-panel{display:none;position:absolute;top:calc(100% + 4px);left:0;min-width:200px;max-height:300px;background:var(--panel);border:1px solid var(--line);border-radius:7px;z-index:200;box-shadow:0 6px 20px rgba(0,0,0,.6);flex-direction:column}
 .ms-panel.open{display:flex}
 .ms-search{border:none;border-bottom:1px solid var(--line);border-radius:7px 7px 0 0;padding:7px 10px;font-size:13px;background:var(--panel);color:var(--text);outline:none;flex-shrink:0}
 .ms-list{overflow-y:auto;flex:1;max-height:240px}
 .ms-item{display:flex;align-items:center;gap:8px;padding:5px 12px;cursor:pointer;font-size:13px;white-space:nowrap;user-select:none}
 .ms-item:hover{background:rgba(255,255,255,.05)}
 .ms-item input[type=checkbox]{cursor:pointer;accent-color:var(--accent);flex-shrink:0;width:14px;height:14px}
 .ms-divider{border:none;border-top:1px solid var(--line);margin:2px 0;flex-shrink:0}
</style></head><body>
...
<div class="controls">
 ...
 <div class="ctrl"><label>Country</label><div class="ms-wrap" id="msWrapCountry"><button class="ms-btn" id="msBtnCountry" type="button">All countries</button><div class="ms-panel" id="msPanelCountry"><input class="ms-search" id="msSearchCountry" placeholder="Search…"><div class="ms-list" id="msListCountry"></div></div></div></div>
 <div class="ctrl"><label>Destination</label><div class="ms-wrap" id="msWrapDest"><button class="ms-btn" id="msBtnDest" type="button">All destinations</button><div class="ms-panel" id="msPanelDest"><input class="ms-search" id="msSearchDest" placeholder="Search…"><div class="ms-list" id="msListDest"></div></div></div></div>
...
// --- Multi-select (Country + Destination) ---
const selCountries=new Set(),selDests=new Set();
const allCountries=uniq('country').sort((a,b)=>a.localeCompare(b));
const allDests2=uniq('dest').sort((a,b)=>a.localeCompare(b));
function buildMS(listId,btnId,searchId,items,sel,allLabel){
  const list=document.getElementById(listId);
  function renderList(q){
    const fi=q?items.filter(x=>x.toLowerCase().includes(q.toLowerCase())):items;
    list.innerHTML=(!q?`<label class="ms-item"><input type="checkbox" class="ms-all-cb" ${sel.size===0?'checked':''}> <span>All / None</span></label><hr class="ms-divider">`:'')
      +fi.map(v=>`<label class="ms-item"><input type="checkbox" value="${v.replace(/"/g,'&quot;')}" ${sel.has(v)?'checked':''}> <span>${v}</span></label>`).join('');
    list.querySelectorAll('input[type=checkbox]').forEach(cb=>{
      cb.addEventListener('change',()=>{
        if(cb.classList.contains('ms-all-cb')){if(cb.checked){sel.clear();}else{items.forEach(v=>sel.add(v));}}
        else{if(cb.checked)sel.add(cb.value);else sel.delete(cb.value);}
        updateMSBtn(btnId,sel,allLabel);render();renderList(document.getElementById(searchId).value);
      });
    });
  }
  document.getElementById(searchId).addEventListener('input',e=>renderList(e.target.value));
  renderList('');updateMSBtn(btnId,sel,allLabel);
}
function updateMSBtn(btnId,sel,allLabel){
  const btn=document.getElementById(btnId);
  btn.textContent=sel.size===0?allLabel:sel.size===1?[...sel][0]:`${sel.size} selected`;
  btn.classList.toggle('active',sel.size>0);
}
function toggleMS(wrapId){
  document.querySelectorAll('.ms-panel').forEach(p=>{if(p.closest('.ms-wrap').id!==wrapId)p.classList.remove('open');});
  const panel=document.querySelector('#'+wrapId+' .ms-panel');
  const opening=!panel.classList.contains('open');
  panel.classList.toggle('open');
  if(opening)document.querySelector('#'+wrapId+' .ms-search').focus();
}
...
document.getElementById('msWrapCountry').querySelector('.ms-btn').addEventListener('click',()=>toggleMS('msWrapCountry'));
document.getElementById('msWrapDest').querySelector('.ms-btn').addEventListener('click',()=>toggleMS('msWrapDest'));
buildMS('msListCountry','msBtnCountry','msSearchCountry',allCountries,selCountries,'All countries');
buildMS('msListDest','msBtnDest','msSearchDest',allDests2,selDests,'All destinations');
...
function filtered(){
  ...
  return DATA.filter(d=>{
    ...
    if(selDests.size&&!selDests.has(d.dest))return false;
...
const COORDS={'Athens':[37.984,23.728],'Barcelona':[41.385,2.173],'Berlin':[52.520,13.405],
 ...
 'Larnaca':[34.900,33.625],'Bucharest':[44.432,26.104],'Sofia':[42.698,23.322],
 'Frankfurt':[50.110,8.682],'Dubai':[25.204,55.271],'Moscow':[55.751,37.618],
...
function renderMap(){
  ...
  const allDests=Object.entries(byDest).sort((a,b)=>a[1][0].price-b[1][0].price);
  const minP=allDests.length?allDests[0][1][0].price:1;
  const maxP=allDests.length?allDests[allDests.length-1][1][0].price:1;
  allDests.forEach(([dest,flights])=>{
    ...
    const t=maxP===minP?1:1-(f0.price-minP)/(maxP-minP);
    const size=6+Math.round(t*8);
    const marker=L.circleMarker(coords,{
      radius:size,fillColor:wColor,color:'#fff',weight:1.5,fillOpacity:0.85,
...
document.getElementById('reset').onclick=()=>{
  ...
  selCountries.clear();selDests.clear();
  buildMS('msListCountry','msBtnCountry','msSearchCountry',allCountries,selCountries,'All countries');
  buildMS('msListDest','msBtnDest','msSearchDest',allDests2,selDests,'All destinations');
  sortKey='price';sortDir=1;render();
};
render();
</script></body></html>'''
